...
@Component({
    ...
    providers: [
        { provide: MatFormFieldControl, useExisting: CdkSampleComponent },
    ],
    host: {
...
export class CdkSampleComponent
    implements
        ControlValueAccessor,
        MatFormFieldControl<BusinessProcessId>,
        OnDestroy {
    static nextId = 0
    formGroup: FormGroup
    ...
    get empty() {
        const {
            value: { businessProcesCoreId, version, processSpecification },
        } = this.formGroup
    ...
    get value(): BusinessProcessId | null {
        ...
        if (
            businessProcesCoreId.length === 3 &&
            version.length === 3 &&
            processSpecification.length === 4
        ) {
            return new BusinessProcessId(
    ...
    set value(businessProcessId: BusinessProcessId | null) {
        const { businessProcesCoreId, version, processSpecification } =
            businessProcessId || new BusinessProcessId('', '', '')
